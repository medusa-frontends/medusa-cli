...
> {
  ...
  getColumns(): Column<T>[] {
    ...
    const widths: Column<T>[] = columns.map((key) => {
      const header = realStringLength(String(key))
      ...
      const data = this.props.data.map((data) => {
        ...
        return realStringLength(String(value))
      })
      ...
      {...intersperse(
        ...
        props.columns.map((column) => {
          ...
          if (value == null) {
            ...
            return <config.cell key={key}>{skeleton.line.repeat(column.width)}</config.cell>
          }
          const key = `${props.key}-cell-${column.key}`

          // margins
          const ml = config.padding
          const mr = column.width - realStringLength(String(value)) - config.padding
